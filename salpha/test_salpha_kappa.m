...
lambda_max_delta = nan(nkappa, nshat, nalpha);
best_theta0_delta = nan(nkappa, nshat, nalpha);
lambda_theta0_delta = nan(nkappa, nshat, nalpha, ntheta0);
fail_message = strings(nkappa, nshat, nalpha, ntheta0);

volume_delta = nan(1, nkappa);
Vprime_delta = nan(1, nkappa);
dVdr_delta = nan(1, nkappa);
q_check_delta = nan(1, nkappa);
minimum_abs_jacobian = nan(1, nkappa);
param_delta = cell(1, nkappa);
...
for ee = 1:nkappa
    delta_tic = tic;
    kappa_value = kappa_grid(ee);
    if ~isfinite(kappa_value) || abs(kappa_value) <= 1
        error('test_salpha_delta:BadDelta', ...
            'Every kappa value must be finite and satisfy abs(delta) > 1.');
    end
    ...
    param_delta{ee} = param_d;

    [volume_delta(ee), Vprime_delta(ee), dVdr_delta(ee)] = ...
        miller_volume_integrals(merluc_d);
    q_check_delta(ee) = merluc_d.q_check;
    minimum_abs_jacobian(ee) = min(abs(merluc_d.jac));

    if minimum_abs_jacobian(ee) <= ...
            1.e-10*max(abs(merluc_d.jac))
        error('test_salpha_kappa:NearlySingularJacobian', ...
            'The Miller coordinate Jacobian is nearly singular at delta=%g.', ...
            kappa_value);
    end

    fprintf(['\nKappa %d/%d: kappa = %.4g, V = %.6g, ', ...
        'Vprime = %.6g, q_check/q = %.8f\n'], ...
        ee, nkappa, kappa_value, volume_delta(ee), Vprime_delta(ee), ...
        q_check_delta(ee)/surf.q);

    for ss = 1:nshat
        row_tic = tic;
        qprime = shat(ss)*(surf.q*Vprime_delta(ee) ...
            /(2*volume_delta(ee)));

        for aa = 1:nalpha
            pprime = -alpha(aa)*(4*pi^2/(2*Vprime_delta(ee))) ...
                *sqrt(2*pi^2*eq.rmaxis/volume_delta(ee))/mu0;
            ...
            for tt = 1:ntheta0
                try
                    ...
                    lambda_theta0_delta(ee, ss, aa, tt) = lambda(1);
                catch ME
                    fail_message(ee, ss, aa, tt) = string(ME.message);
                end
            end

            values = reshape( ...
                lambda_theta0_delta(ee, ss, aa, :), 1, []);
            [lambda_max_delta(ee, ss, aa), imax] = ...
                max(values, [], 'omitnan');
            if isfinite(lambda_max_delta(ee, ss, aa))
                best_theta0_delta(ee, ss, aa) = theta0(imax);
            end
        end

        completed_rows = (ee - 1)*nshat + ss;
    ...
    fprintf('Kappa %.4g scan finished in %.1f min.\n', ...
        kappa_value, toc(delta_tic)/60);
end
...
salpha_delta_result.delta = kappa_grid;
