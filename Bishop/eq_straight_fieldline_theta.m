...
%   'Direction'    : 'counterclockwise', 'clockwise', or 'as-is'.
...
%   cont.JacMode, cont.integrand
    ...
    addParameter(p, 'Direction', 'counterclockwise', @(x)ischar(x) || isstring(x));
    parse(p, varargin{:});
    opt = p.Results;
    ...
    if ~any(direction == ["as-is", "clockwise", "counterclockwise"])
        error('eq_straight_fieldline_theta:BadDirection', ...
...
function [gradPsi, Bpol] = local_grad_bpol(~, Rgrid, Zgrid, psiRZ)
    % psiRZ is nR x nZ. MATLAB's first gradient output differentiates
    % along the column coordinate, so the spacing order is (Z, R).
